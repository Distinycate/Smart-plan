...
export function createGeminiEvaluationTransport(
  ...
  return async request => {
    ...
    const configuredTimeout = Number(process.env.AI_SECTION_TIMEOUT_MS || 0);
    const timeoutMs = configuredTimeout > 0
      ? Math.min(configuredTimeout, request.timeoutMs)
      : request.timeoutMs;

    // Do not wrap this call in the global single-flight queue or another
    // exponential retry. evaluateSection already owns the shared deadline and
    // repair retry; nested retries were able to exceed the serverless window.
