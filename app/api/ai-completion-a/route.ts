...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `MASTER SYSTEM PROMPT V1 (STEP 2: COMPLETION - A RUBRIC)
...
หน้าที่ของคุณคือ "สร้างเกณฑ์การประเมินด้านคุณลักษณะ (A)" ให้สอดคล้องกับกระบวนการเรียนรู้และจุดประสงค์ที่กำหนดไว้อย่างสมบูรณ์แบบที่สุด

หลักการสำคัญ
1. การวัดผล (Measure A) ต้องตอบโจทย์จุดประสงค์การเรียนรู้ A ด้านบนอย่างแม่นยำ
...
1. measureA: (สิ่งที่ต้องการวัดและประเมินผล A)
2. methodA: (วิธีการวัด A)
3. toolA: (เครื่องมือวัด A)
4. criteriaA: (เกณฑ์ประเมิน A - ระบุระดับคะแนนที่ผ่าน)
5. rubricA: (คำอธิบายเกณฑ์ Rubric A แบ่งเป็น 5 ระดับอย่างละเอียด: 5, 4, 3, 2, 1 ห้ามตัดตอน)`;
    ...
    const response = await fetchGeminiWithRetry(apiUrl, payload, 3, apiKey, 'completion-a');
